...
def pegasus_paraphrase(texts, 
    ...
    def paraphrase(sents):
        ...
        paraphrased = [paraphraser_tokenizer.decode(paraphrased_ids[i], skip_special_tokens=True) for i in range(len(paraphrased_ids))]       
    ...
    name = args.data_path + \
        f'-pegasus-bigram={bigram}-num_beams={num_beams}-threshold={bert_threshold}'
    print("Saving results to", name)
    new_dataset.save_to_disk(name)
    return output

def parrot_paraphrase(parrot, texts, tokenizer, num_beams=25, bigram=False, save_to_disk=True, save_by_sents=False, bert_threshold=0.03):
    ...
    name = args.data_path + \
        f'-parrot-bigram={bigram}-num_beams={num_beams}-threshold={bert_threshold}'
    print("Saving results to", name)
    new_dataset.save_to_disk(name)
    pkl_name = args.data_path + f'-parrot-bigram={bigram}-num_beams={num_beams}-threshold={bert_threshold}-all_beams.pkl'
    with open(pkl_name, 'wb') as f:
        pickle.dump(total_paraphrased, f)
        f.close()
    return output

def paraphrase_openai(client, texts, num_beams=10, bigram=False):
    new_texts = []
    all_paras = []
    MAX_ITER = 10
        ...
        for i in range(len(sents)):
            ...
            if bigram:
                ...
                while(len(para_ls) < 5 and num_iter < MAX_ITER):
                    para_str = query_openai_bigram(client, prompt, sent)
                    # use regex to extract list from string
                    para_ls = extract_list(para_str)
                    num_iter += 1
                    # openai refuses to paraphrase, then discard
                ...
                para = query_openai(client, prompt, sent)
                para_sents.append(para)
        if not fail:
            new_texts.append(sents)
            all_paras.append(para_sents) 
    
    save_path = args.data_path + f'-gpt-num_beams={num_beams}-bigram={bigram}'
...
if __name__ == '__main__':
    ...
    parser.add_argument('--bert_threshold', type=float, default=0.1, help='threshold for bert similarity between original and paraphrased')
    parser.add_argument('--num_beams', type=int, default=25, help='number of beams for beam-search')
    parser.add_argument('--api_key', type=str, default='', help='API key for OpenAI')
    ...
    if args.paraphraser == 'parrot':
        ...
        pegasus_paraphrase(texts, tokenizer, bigram=True, num_beams=args.num_beams, bert_threshold=args.bert_threshold, device=device)
    elif args.paraphraser == 'pegasus':
        pegasus_paraphrase(texts, tokenizer, num_beams = args.num_beams, bert_threshold=args.bert_threshold, bsz=args.bsz, device=device)
    elif args.paraphraser == 'gpt':
        client = openai.OpenAI(api_key=args.api_key)
        new_texts, paras = paraphrase_openai(client, texts, args.num_beams, bigram=False)
    elif args.paraphraser == 'gpt-bigram':
        client = openai.OpenAI(api_key=args.api_key)
