...
def run_bert_score(gen_sents, para_sents):
    _, _, F1 = scorer.score(gen_sents, para_sents)
...
def accept_by_bigram_overlap(sent, para_sents, tokenizer, bert_threshold):
...
def query_openai(client, prompt, sent):
  while True:
    try:
      ...
      print("API error", e)
      if ("different prompt" in e.message):
         return sent
      else:
        continue
    break
...
# use long context
@backoff.on_exception(backoff.expo, openai.RateLimitError)
def query_openai_bigram(client, prompt, sent):
  while True:
    try:
      response = client.chat.completions.create(
