...
import argparse
import os
from eval_util import load_file_by_line, path_wo_ext, break_text, chunks
...
def eval_perplexity(model, tokenizer, texts, generation_file=None, K=500, name_suffix=''):
    ...
    for i, text in enumerate(tqdm(texts, desc='perplexity')):
        ...
        # prefix = ''
        input_ids = tokenizer.encode(
            prefix + text, return_tensors='pt', truncation=True, padding=True, max_length=model.config.max_position_embeddings).to(device)
